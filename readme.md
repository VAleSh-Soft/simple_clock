...
Для матричных экранов доступен вывод даты в виде бегущей строки, для этого нужно раскомментировать строку `#define USE_TICKER_FOR_DATE` в файле **matrix_data.h**. Скорость бегущей строки в кадрах в секунду определяется строкой `#define TICKER_SPEED` в файле **matrix_data.h**. Чем чаще сменяются кадры, тем быстрее движется строка (строка даты занимает примерно 190 кадров).
